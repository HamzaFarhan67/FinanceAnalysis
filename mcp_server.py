...
from .rag import split_markdown_into_chunks   # Course chunker
...
def seed_existing_data(user_id: int, source_name: str, markdown_text: str):
    db = SessionLocal()
    try:
        # 1. Use course chunker from Week 7 to get Pydantic objects
        pydantic_chunks = split_markdown_into_chunks(source_name, markdown_text)
        
        # 2. Embed and save into PostgreSQL pgvector table
        for chunk in pydantic_chunks:
            vec = get_embedding(chunk.content)
            
            # Map Pydantic object to SQLAlchemy DB Model
